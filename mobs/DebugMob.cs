...
namespace BridgeTroll
{
    public partial class DebugMob : CharacterBody2D
    {
        ...
        {
            if (Position.DistanceTo(goal_target_) < 20)
            {
            ...
            {
                Vector2 calculated_velocity =
                    Position.DirectionTo(navigation_agent_.GetNextPathPosition()) * speed;
                if (navigation_agent_.AvoidanceEnabled)
                {
                    navigation_agent_.Velocity = calculated_velocity;
                }
                else
                {
        ...
        void _on_navigation_agent_2d_velocity_computed(Vector2 safe_velocity)
        {
